...
int main(){
    ...
    {double *C=C_no;for(int kk=0;kk<K;kk+=64)for(int ii=0;ii<M;ii+=64)for(int jj=0;jj<N;jj+=64)for(int k=kk;k<kk+64&&k<K;k++)for(int i=ii;i<ii+64&&i<M;i++)for(int j=jj;j<jj+64&&j<N;j++)C[i*N+j]+=A[i*K+k]*B[k*N+j];;}
    ...
    clock_gettime(CLOCK_MONOTONIC,&s);
    {double *C=C_yes;for(int kk=0;kk<K;kk+=64)for(int ii=0;ii<M;ii+=64)for(int jj=0;jj<N;jj+=64)for(int k=kk;k<kk+64&&k<K;k++)for(int i=ii;i<ii+64&&i<M;i++)for(int j=jj;j<jj+64&&j<N;j++)C[i*N+j]+=A[i*K+k]*B_T[j*K+k];;}
    clock_gettime(CLOCK_MONOTONIC,&e);
    ...
    printf("block_kij<elem_jki|%.2f|%.2f|%.2f|%.2f|%.2fx|%+.2f%%\n",t_no,t_total,t_trans,t_mul,speedup,improv);
